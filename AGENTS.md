...
```ts
import { buildFiveOhOneSession, type FiveOhOneSession } from "@lib/shared/games/501";
```
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!auth.isLoggedIn || !auth.userId) return jsonResponse({ ok: false, code: MessageCode.UNAUTHORIZED }, 401);
  // parse { session } wrapper or raw session
  const validated = validateCompleted*Session(sessionPayload);
  if (!validated.valid) return jsonResponse({ ok: false, code: validated.code }, 400);
...
**Entry:** `lib/client/alpine/app.factory.ts` (referenced in `astro.config.mjs`).
