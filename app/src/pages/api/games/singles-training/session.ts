...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    const existing = await getSinglesTrainingSession(auth.userId);
    if (existing) {
      return jsonResponse({ ok: false, code: MessageCode.SESSION_EXISTS }, 409);
    }

    const created = await createSinglesTrainingSession(auth.userId, validated.value);
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    const active = await getSinglesTrainingSession(auth.userId);
    if (!active) {
      return jsonResponse(
