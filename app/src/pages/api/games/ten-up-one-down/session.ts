...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    const existing = await getTenUpOneDownSession(auth.username);
    if (existing) {
      return jsonResponse({ ok: false, code: MessageCode.SESSION_EXISTS }, 409);
    }

    const created = await createTenUpOneDownSession(auth.username, validated.value);
...
export const GET: APIRoute = async ({ cookies }) => {
  ...
  try {
    const active = await getTenUpOneDownSession(auth.username);
    if (!active) {
      return jsonResponse(
