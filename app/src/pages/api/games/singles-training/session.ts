...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    const existing = await getSinglesTrainingSession(auth.username);
    if (existing) {
      return jsonResponse({ ok: false, code: MessageCode.SESSION_EXISTS }, 409);
    }

    const created = await createSinglesTrainingSession(auth.username, validated.value);
...
export const GET: APIRoute = async ({ cookies }) => {
  ...
  try {
    const active = await getSinglesTrainingSession(auth.username);
    if (!active) {
      return jsonResponse(
